...
export default function ProjectsShowcase({ translations }: ProjectsShowcaseProps) {
   ...
   const goLeft = useCallback((): void => {
      ...
      setRealIndex(prev => {
         ...
         // เงื่อนไขการวาปสำหรับ goLeft
         // ถ้า next น้อยกว่า totalProjects (ซึ่งหมายถึงกำลังจะไปถึงชุดการ์ดแรก)
         // ให้วาปไปที่ตำแหน่งที่เหมาะสมในชุดการ์ดที่ซ้ำกัน (เช่น ชุดที่ 3 จาก 4)
         if (next < totalProjects) {
            resetTimeoutRef.current = setTimeout(() => {
               // วาปไปตำแหน่งก่อนหน้าที่เคยเป็น next + totalProjects * 2
               // เพื่อให้การ์ดชุดเดียวกันปรากฏขึ้นมาแทน
               setRealIndex(next + totalProjects * 2); // เพิ่ม 2 ชุดเต็มเพื่อ "วาป" ข้ามไป
   ...
   const goRight = useCallback((): void => {
      ...
      setRealIndex(prev => {
         ...
         // เงื่อนไขการวาปสำหรับ goRight
         // ถ้า next มากกว่าหรือเท่ากับตำแหน่งที่อยู่ใกล้ชุดการ์ดสุดท้าย
         // ให้วาปกลับไปที่ตำแหน่งเริ่มต้นของชุดที่ 2 เพื่อให้เลื่อนต่อไปได้
         if (next >= infiniteProjects.length - totalProjects) {
            resetTimeoutRef.current = setTimeout(() => {
               setRealIndex(totalProjects); // วาปกลับไปที่เริ่มต้นของชุดที่ 2
               resetTimeoutRef.current = null;
            }, 600);
         }
         return next
